...
long long __FILETIME_to_ll (const FILETIME *f)
{
  long long t;
...
void __ll_to_FILETIME (long long t, FILETIME* f)
{
  f->dwHighDateTime = (DWORD)((t >> 32) & 0x00000000FFFFFFFF);
  f->dwLowDateTime = (DWORD)(t & 0x00000000FFFFFFFF);
}

void __tm_to_SYSTEMTIME (struct tm *t, SYSTEMTIME *s)
{
...
static void __FILETIME_from_Year (WORD year, FILETIME *f)
{
  SYSTEMTIME s = {0};
...
static int __Yday_from_SYSTEMTIME (const SYSTEMTIME *s)
{
  long long t;
  FILETIME f1, f2;
...
void __SYSTEMTIME_to_tm (SYSTEMTIME *s, struct tm *tm)
{
...
time_t __FILETIME_to_time_t (const FILETIME* f)
{
  long long t;
...
void __time_t_to_FILETIME (time_t t, FILETIME *f)
{
  long long time;
